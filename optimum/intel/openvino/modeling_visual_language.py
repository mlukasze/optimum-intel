...
class _OVMolmo2ForCausalLM(OVModelForVisualCausalLM):
    """OpenVINO runtime for the native Molmo2 VLM (e.g. allenai/MolmoWeb-4B).

    The vision stack consumes patchified crops ``pixel_values`` plus
    ``image_token_pooling`` indices (built by the image processor for the
    multi-crop tiling + 2x2 pooling scheme) and returns per-token pooled
    features that are additively merged into the ``<im_patch>`` placeholder
    positions of the text embeddings (unlike replace-style VLMs).
    """
