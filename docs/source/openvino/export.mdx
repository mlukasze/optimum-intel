...
For remote-code visual language models such as `nvidia/Nemotron-Labs-Diffusion-VLM-8B`, pass the task explicitly and enable remote code:
...
Known limitations for this architecture:
- the language backbone uses bidirectional diffusion instead of autoregressive decoding
- `use_cache=False`, so generation-oriented flows may require model-specific handling
- inference should use the model processor chat template from the original repository
